...
class Controls:
  ...
  def update_events(self, CS):
    ...
    if not REPLAY:
      ...
      cruise_mismatch = CS.cruiseState.enabledAcc and (not self.enabled)
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(CC.longActive and CS.cruiseState.enabledAcc, CS, self.CP, long_plan, pid_accel_limits, t_since_plan, self.sm['radarState'])
      ...
      if CS.vEgo < 0.3:
        actuators.steeringAngleDeg = float(CS.steeringAngleDeg)
      else:
        actuators.steeringAngleDeg = (math.degrees(self.VM.get_steer_from_curvature(-desired_curvature, CS.vEgo, params.roll)) * 180) / 200
