...
from selfdrive.car.hyundai.values import CAR, EV_CAR, HYBRID_CAR, LEGACY_SAFETY_MODE_CAR, Buttons, CarControllerParams, FEATURES
...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if candidate in [CAR.SANTA_FE, CAR.SANTA_FE_2022, CAR.SANTA_FE_HEV_2022]:
      ...
      ret.steerRatio = 12.35 # sunnyhaibin's personal tune - 2021 Sonata N Line
      tire_stiffness_factor = 0.65
      ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kpBP, ret.lateralTuning.pid.kdBP = [[0.], [0.], [0.]] # sunnyhaibin's personal tune - 2021 Sonata N Line
      ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV, ret.lateralTuning.pid.kdV = [[0.25], [0.05], [2.5]] # sunnyhaibin's personal tune - 2021 Sonata N Line
  ...
  def update(self, c, can_strings):
    ...
    if self.CS.disengageByBrake and not ret.brakePressed and not ret.brakeHoldActive and (self.CS.lfaEnabled or self.CS.accMainEnabled):
      enable_pressed = True
  ...
  def apply(self, c):
    can_sends = self.CC.update(c.enabled, self.CS, self.frame, c.actuators,
                               ...
                               c.hudControl.rightLaneVisible, c.hudControl.leftLaneDepart, c.hudControl.rightLaneDepart, c.hudControl.leadVisible)
    self.frame += 1
    return can_sends
